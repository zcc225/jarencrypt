...
//                  bytes = ByteCodeEncryptor.encrypt(bytes);
            	  bytes =   Use3DES.encrypt("01234567899876543210abcd".getBytes(),bytes);
              } catch (Exception e) {
                  e.printStackTrace();
              }
          }
  ...
  public static void decode(String fileName){
    try {
      ...
      byte[] buf = new byte[1024];
      File srcFile = new File(fileName);
      File dstFile = new File(fileName.substring(0, fileName.indexOf("."))+"_decoded.jar");
      FileOutputStream dstFos = new FileOutputStream(dstFile);
      JarOutputStream dstJar = new JarOutputStream(dstFos);
      JarFile srcJar = new JarFile(srcFile);
      for (Enumeration<JarEntry> enumeration = srcJar.entries(); enumeration.hasMoreElements();) {
          ...
          if(name.endsWith(".class")){
              try {
//                  bytes = ByteCodeEncryptor.encrypt(bytes);
            	  bytes =   Use3DES.decrypt("01234567899876543210abcd".getBytes(),bytes);
              } catch (Exception e) {
                  e.printStackTrace();
              }
          }
